...
public class AIPlayer extends Player {
	...
	public AIAction getAction() {
		...
		for (int i = 0; i < 1000; i++) {
			...
			gameLose.put(rank.get(rank.size() - 1).getColor(),
					gameLose.getOrDefault(rank.get(rank.size() - 1).getColor(), 0) + 1);
		}
		...
		for (RaceBettingCard rbc : getRaceBets()) {
			...
			double winEV = (chanceFirstWinPlace * chanceWin * 8 - 1 * (1 - chanceWin))
					* raceWinMult(g.getTrack().getCamelPos(g.getCamelRankings().get(0)));
			if (winEV > eValue) {
				eValue = winEV;
	...
	public double raceLoseMult(int secondToLastCamelPos, int lastCamelPos) {
		return Math.pow(Math.min(1, (secondToLastCamelPos - lastCamelPos) / 4.0), 2);
	}
