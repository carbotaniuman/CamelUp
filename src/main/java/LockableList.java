...
/**
 * Created by IntelliJ IDEA. User: francoiscassistat Date: 12 juin 2010 Time:
 * 18:20:18
 *
 *
 * Lockable list, made to debug ConcurrentModificationException on Lists. The
 * lock can be switched on/off with setLocked(boolean). When locked, all write
 * access to the list or iterators gets ConcurrentModificationException. Simple
 * usage case :
 *
 * list.setLocked(true);
 *
 * for (Object o : list.iterator()) // now this won't get
 * ConcurrentModificationException, the other instruction that cause this will
